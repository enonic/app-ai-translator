...
export const isInput = <T extends FormItem>(item: T): item is T & FormItemInput =>
  item.formItemType === 'Input';

export const isFormItemSet = (item: FormItem): item is FormItemSet =>
  item.formItemType === 'ItemSet';

export const isFieldSet = (item: FormItem): item is FormItemLayout =>
  item.formItemType === 'Layout';

export const isFormItemOptionSet = (item: FormItem): item is FormItemOptionSet =>
  item.formItemType === 'OptionSet';

export const isFormFragment = (item: FormItem): item is FormItemFormFragment =>
  item.formItemType === 'FormFragment';
...
function fetchFormItemPath(item: FormItem, path: Path): FormItemWithPath[] {
  ...
  if (isFormFragment(item)) {
    return getFormFragmentPathEntries(item, path);
  }
...
function getFormItemSetPathEntries(item: FormItemSet, path: Path): FormItemWithPath[] {
  ...
  return [
    ...
    ...getPathsOfMentionableItems(item.items, { elements: pathElements }),
  ];
}
